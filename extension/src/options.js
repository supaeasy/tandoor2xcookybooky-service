...
async function save() {
  ...
  if (!tandoorOrigin || !backendOrigin) {
    status.textContent = "Bitte gültige URLs eingeben (inkl. http:// oder https://).";
    return;
  }

  status.textContent = "Berechtigungen werden angefragt …";
  const granted = await chrome.permissions.request({
    origins: [`${tandoorOrigin}/*`, `${backendOrigin}/*`],
  });
  if (!granted) {
    status.textContent = "Berechtigung wurde nicht erteilt – Einstellungen nicht gespeichert.";
    return;
  }
  ...
  status.textContent = "Gespeichert! Lade die Rezeptseite neu, damit der Button erscheint.";
}
